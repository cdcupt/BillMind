...
/// explicit date is present, `date` is left `nil` so the client's Today / Yesterday
/// clarify (which knows the real local day) resolves it.
public enum DraftExtractor {
    ...
    /// resolved here; the client clarify handles those with the real local day.
    public static func date(in raw: String, reference: Date = Date()) -> Date? {
    ...
    public static func rawDateText(in raw: String, reference: Date = Date()) -> String? {
    ...
    public static func merchant(in raw: String, reference: Date = Date()) -> String? {
        var text = raw
